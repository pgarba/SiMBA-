...
def main():
    ...
    filenames = [f for f in os.listdir('../data') if re.match(r'^bitwise_list_.*\.txt$', f)]    
    ...
    for filename in filenames:
        ...
        vector = ", ".join(["\"" + line.split("#")[0].strip() + "\"" for line in open('../data/' + filename, 'r')])

        # Get filename without extension
        filename_without_extension = os.path.splitext(filename)[0]

        print ("std::vector<std::string> " + filename_without_extension + " = {" + vector + "};")                  
